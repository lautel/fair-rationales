...
def get_adjmat(
    mat: np.ndarray, input_tokens: List[int]
...
def get_flow_relevance(full_att_mat, input_tokens, layer):
    ...
    res_adj_mat, res_labels_to_index = get_adjmat(
        mat=res_att_mat, input_tokens=input_tokens
    )
    ...
    length = full_att_mat.shape[-1]
    final_layer_attention_raw = flow_values[
        (layer + 1) * length : (layer + 2) * length,
...
def get_flow_relevance_for_all_layers(encoded, x, tokens, layers, pad_token):
    ...
    for l in layers:
        ...
        attention_relevance = np.concatenate(
            (attention_relevance, np.array([-1.0] * (sen_len - valid_token_len))),
            axis=None,
        )
